...
function describeError(err: unknown): string {
  if (err instanceof Error) {
    ...
    if (msg.includes("Could not connect") || msg.includes("Failed to fetch") || msg.includes("fetch failed")) {
    ...
    try {
      ...
      if (result.signingIn) {
        ...
        setView({
          name: "check",
          email: email.trim().toLowerCase(),
          message: "We emailed a verification link to your address. Check your inbox to finish signing in.",
        });
      }
    } catch (err) {
    ...
    try {
      ...
      if (result.signingIn) {
        ...
        setView({
          name: "check",
          email: email.trim().toLowerCase(),
          message: "Almost done — we emailed a verification link to your address. Click it to activate your account.",
        });
      }
    } catch (err) {
      setError(describeError(err));
      ...
      <DialogContent>
        <DialogHeader>
          <div className="flex items-center gap-md">
            <Image src="/logo.png" alt="" width={794} height={672} style={{ height: '40px', width: 'auto' }} className="rounded-lg" />
            <div>
        ...
        {error && (
          <p role="alert" className="rounded-xl border border-error/40 bg-error/10 px-md py-sm text-body-md text-error">
            {error}
          </p>
        )}
          ...
          <form onSubmit={handlePasswordSignIn} className="space-y-md">
            ...
            <div className="flex flex-col items-center gap-sm text-body-md">
              ...
              <p className="text-on-surface-variant">
                New to Event Nu?{" "}
                <button type="button" className="text-primary hover:underline" onClick={() => switchTo({ name: "signup" })}>
                  Create an account
                </button>
              </p>
            </div>
          ...
          <form onSubmit={handleSignUp} className="space-y-md">
            ...
            <label className="flex items-start gap-sm text-body-md text-on-surface-variant">
              ...
              <span>
                ...
                and{" "}
                <Link href={PRIVACY_POLICY_URL} className="text-primary hover:underline" target="_blank">
                  Privacy Policy
                </Link>
                .
              </span>
            ...
            <p className="text-center text-body-md text-on-surface-variant">
              ...
              <button type="button" className="text-primary hover:underline" onClick={() => switchTo({ name: "signin" })}>
                Sign in
              </button>
            </p>
          </form>
        ...
        {view.name === "check" && (
          <div className="space-y-md">
            ...
            <p className="text-body-md text-on-surface-variant">
              Didn&apos;t get it?{" "}
              <button type="button" className="text-primary hover:underline disabled:opacity-50" onClick={handleResend} disabled={loading}>
                Resend email
              </button>{" "}
              or{" "}
              <button
            ...
            <Button type="button" variant="ghost" className="w-full" onClick={() => switchTo({ name: "signin" })}>
              Back to sign in
            </Button>
          </div>
        )}
        ...
        {view.name === "code" && (
          <form onSubmit={handleCodeSubmit} className="space-y-md">
            <p className="text-body-md text-on-surface-variant">
              Enter the code we emailed to <span className="font-mono text-on-surface">{view.email}</span>.
            </p>
            <div className="space-y-sm">
